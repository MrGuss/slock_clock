...
static void
writeclock(Display *dpy, Window win, int screen)
{
...
static void *displayClock(void *arguments)
{
	struct arg_clock *args = arguments;
	int nscreens = args->nscreens;
	struct lock **locks = args->locks;
	Display *dpy = args->dpy;
	int screen;
	while (true)
	{
...
static void
	*
	readpw(void *arguments)
{
	struct arg_readpw *args = arguments;
	Display *dpy = args->dpy;
	struct xrandr *rr = args->rr;
	struct lock **locks = args->locks;
	int nscreens = args->nscreens;
...
int main(int argc, char **argv)
{
	...
	pthread_t id1;
	pthread_t id2;
	struct arg_clock argsc;
	argsc.dpy = dpy;
	argsc.locks = locks;
	argsc.nscreens = nscreens;
	struct arg_readpw argsr;
	argsr.dpy = dpy;
	argsr.rr = &rr;
	argsr.locks = locks;
	argsr.nscreens = nscreens;
	argsr.hash = hash;
	pthread_create(&id1, NULL, readpw, (void *)&argsr);
	pthread_create(&id2, NULL, displayClock, (void *)&argsc);
